...
use std::fmt::Display;
use std::io::{self, stdout, BufRead, BufReader, ErrorKind, Read, Seek, Write};
...
use crate::{config::Config, error::Error, syntax::Conf as SyntaxConf, terminal as term};
...
impl Editor {
    ...
    fn loop_until_keypress(&mut self) -> Result<Key, Error> {
        loop {
                ...
                if let Event::Key(key) = event::read()? {
                    return Ok(match key {
                        KeyEvent {
                            ...
                            ..
                        } => Key::CtrlArrow(AKey::Up),
                        KeyEvent {
                            code: KeyCode::Down,
                            modifiers: KeyModifiers::CONTROL,
                            ..
                        } => Key::CtrlArrow(AKey::Down),
                        KeyEvent {
                            code: KeyCode::Left,
                            modifiers: KeyModifiers::CONTROL,
                            ..
                        } => Key::CtrlArrow(AKey::Left),
                        KeyEvent {
                            code: KeyCode::Right,
                            modifiers: KeyModifiers::CONTROL,
                            ..
            ...
            if term::get_window_size()? != (self.window_width as u16, (self.screen_rows + 2) as u16)
            {
    ...
    fn update_window_size(&mut self) -> Result<(), Error> {
        let (cols, rows) = term::get_window_size()?;
        (self.screen_rows, self.window_width) = (rows.saturating_sub(2) as usize, cols as usize);
    ...
    fn draw_left_padding<T: Display>(&self, val: T) -> Result<(), Error> {
        if self.ln_pad >= 2 {
            queue!(
                stdout(),
                Print(format!("{:>width$} │", val, width = self.ln_pad - 2).dark_grey()),
            )?;
        }
        Ok(())
    }
    ...
    fn draw_rows(&self) -> Result<(), Error> {
        ...
        for (i, row) in row_it.skip(self.cursor.roff).take(self.screen_rows) {
            ...
            if let Some(row) = row {
                // Draw a row of text
                self.draw_left_padding(i + 1)?;
                row.draw(self.cursor.coff, self.screen_cols)?;
            } else {
                // Draw an empty row
                self.draw_left_padding('~')?;
                if self.is_empty() && i == self.screen_rows / 3 {
                    let welcome_message = "QwQ";
                    write!(stdout(), "{:^1$.1$}", welcome_message, self.screen_cols)?;
                }
            }
            write!(stdout(), "\r\n")?;
        }
    ...
    fn draw_status_bar(&self) -> Result<(), Error> {
        ...
        write!(
            stdout(),
            "{}\r\n",
            format!("{left}{:>rw$.rw$}", right).reverse()
        )?;
        Ok(())
    }
    ...
    fn draw_message_bar(&self) -> Result<(), Error> {
            ...
            .filter(|sm| sm.time.elapsed() < msg_duration)
        {
            write!(
                stdout(),
                "{}",
                &sm.msg[..sm.msg.len().min(self.window_width)]
            )?;
        }
        Ok(())
    }
    ...
    fn refresh_screen(&mut self) -> Result<(), Error> {
        ...
        if self.prompt_mode.is_none() {
            queue!(
                std::io::stdout(),
                cursor::MoveTo(
                    (self.rx() - self.cursor.coff + 1 + self.ln_pad) as u16,
                    (self.cursor.y - self.cursor.roff + 1) as u16
                )
            )?;
        } else {
            queue!(
                std::io::stdout(),
                cursor::MoveTo(
                    self.status_msg.as_ref().map_or(0, |sm| sm.msg.len() + 1) as u16,
                    (self.screen_rows + 2) as u16
                )
            )?;
        }

        queue!(std::io::stdout(), cursor::Show)?;
        std::io::stdout().flush()?;
    ...
    pub fn run(&mut self, file_name: &Option<String>) -> Result<(), Error> {
        if let Some(path) = file_name.as_ref().map(|p| PathBuf::from(p)) {
...
impl Drop for Editor {
    fn drop(&mut self) {
        terminal::disable_raw_mode().expect("Could not restore terminal mode");
        execute!(
            std::io::stdout(),
            terminal::LeaveAlternateScreen,
            cursor::Show
        )
        .expect("Error restoring terminal");
    }
}
