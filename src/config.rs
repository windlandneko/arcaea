...
{
    let file = File::open(path).map_err(|e| Error::Config(path.into(), 0, e.to_string()))?;
    for (i, line) in BufReader::new(file).lines().enumerate() {
        ...
        match (parts.next(), parts.next()) {
            ...
            (Some(k), Some(v)) => {
                kv_fn(k.trim_end(), v).map_err(|r| Error::Config(path.into(), i, r))?
            }
            (Some(""), None) | (None, _) => (), // Empty line
            (Some(_), None) => return Err(Error::Config(path.into(), i, String::from("No '='"))),
        }
    }
    Ok(())
}
...
/// Returns a list of directories where configuration files might be located.
pub fn config_dirs() -> Vec<String> {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        dirs.push("/etc/arcaea".to_string());

        // User-level configuration
        if let Some(config_home) = std::env::var_os("XDG_CONFIG_HOME") {
            dirs.push(format!("{}/arcaea", config_home.to_string_lossy()));
        } else if let Some(home) = std::env::var_os("HOME") {
            dirs.push(format!("{}/.config/arcaea", home.to_string_lossy()));
        }
    }
    ...
    #[cfg(target_os = "windows")]
    {
        if let Some(appdata) = std::env::var_os("APPDATA") {
            dirs.push(format!("{}/arcaea", appdata.to_string_lossy()));
        }
    }
